package com.epam.tc.hw3.utils;

public class URI {

    public static final String DOMAIN_URI = "https://api.trello.com/";
    public static final String BOARDS_END_POINT = "/1/boards";
    public static final String USER_BOARD_END_POINT = "/1/boards/%s";
    public static final String LISTS_END_POINT = "/1/lists/";
    public static final String USER_LISTS_END_POINT = "/1/lists/%s";
    public static final String CARDS_END_POINT = "/1/cards/";
    public static final String USER_CARDS_END_POINT = "/1/cards/%s";
    public static final String CARD_STICKERS_END_POINT = "/1/cards/%s/stickers";
}
